-- HR document template library + per-employee assignments + multi-signer
-- tracking. Templates are seeded separately (see the companion seed
-- migration) from the 53-doc company guide. owner_only on the template is
-- the single chokepoint for "managers get HR forms, not ops/financial
-- docs" — every downstream read (template list, send-document picker,
-- future full-guide browser) inherits it for free.
...
-- Snapshot-on-edit, mirrors sop_versions exactly.
...
-- Per-employee "this document, sent to this person" record. When sent from
-- a template, title/body_blocks/required_signer_roles are copied at
-- send-time (immutable snapshot) so a later template edit never
-- retroactively changes a document someone already signed.
...
-- One row per required signer per assignment, auto-created from
-- required_signer_roles by the trigger below. A non-employee role label
-- (e.g. "Manager Issuing Warning") can be signed by any manager/owner; the
-- "Employee Signature" row can only be signed by employee_id — enforced in
-- the signHrDocument() server function (Task 5), not here, since the role
-- check needs has_role()/is_manager() context per-row rather than a static
-- RLS predicate on signer_role_label text.
