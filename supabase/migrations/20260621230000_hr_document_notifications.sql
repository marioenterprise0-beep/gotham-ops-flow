-- HR document notifications: two new alert types (the "assigned" alert is
-- inserted directly by assignHrDocument(); the "signed" alert is inserted
-- here, extending the existing completion trigger, since that trigger is
-- the single source of truth for "every required signature is in").
...
BEGIN
  IF NEW.signed_at IS NOT NULL AND OLD.signed_at IS NULL THEN
    ...
    IF remaining = 0 THEN
      ...
      -- assigned_user_id targets the MANAGER who should be notified that
      -- everything is signed — created_by records whose completion
      -- triggered it (the employee), mirroring emit_profile_milestone_alert's
      -- convention of created_by = "who this alert is about".
