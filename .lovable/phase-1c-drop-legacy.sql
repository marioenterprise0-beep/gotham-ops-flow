-- Phase 1c: Rewrite remaining legacy-helper callers and drop the legacy overloads.
--
-- Rewrites two SECURITY DEFINER trigger functions and seven RLS policies to use
-- the explicit org-scoped helpers, then drops:
--   * public.is_manager(_user_id uuid)
--   * public.has_role(_user_id uuid, _role app_role)
--   * public.my_active_org_roles()

BEGIN;

-- 1. Rewrite SECURITY DEFINER trigger functions -----------------------------
...
-- 2. Rewrite RLS policies referencing legacy helpers ------------------------
...
-- 4. Assert the drops actually happened -------------------------------------
