...
} from "@/lib/archive-center.functions";
...
function ArchiveCenterPage() {
  ...
  const {
    ...
    queryFn: () =>
      fetchList({ data: { table: activeTable, search: search || undefined, limit: 100 } }),
  });
  ...
  const deleteM = useMutation({
    mutationFn: ({ id, force }: { id: string; force: boolean }) =>
      doDelete({ data: { table: activeTable, id, force } }),
    onSuccess: () => {
    ...
    onError: (e: any) => {
      if (e?.message === "HAS_DEPENDENCIES")
        toast.error("Has live dependencies — review before forcing");
  ...
  return (
    <AppShell>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search archived rows…"
                value={search}
          ...
          <Card className="p-0 overflow-hidden">
            ...
            {!isLoading && rows.length === 0 && (
              <div className="p-6 text-sm text-muted-foreground text-center">No archived rows.</div>
            )}
            {rows.map((r: any, i: number) => (
              <div
                key={r.id}
                className={cn(
                  "p-3 grid grid-cols-1 md:grid-cols-[1fr_160px_180px_auto] gap-2 items-center",
              ...
              >
                <div className="min-w-0">
                  ...
                  {r.archive_reason && (
                    <div className="text-xs text-muted-foreground truncate">{r.archive_reason}</div>
                  )}
                </div>
                <div className="text-xs text-muted-foreground">
                  {new Date(r.archived_at).toLocaleDateString()}{" "}
                  {new Date(r.archived_at).toLocaleTimeString([], {
                    hour: "numeric",
                    minute: "2-digit",
                  })}
                </div>
                ...
                <div className="flex gap-2 justify-end">
                  <Button
                    ...
                    variant="ghost"
                    onClick={() =>
                      setDepsRow({ table: activeTable, id: r.id, name: r.display_name })
                    }
                  >
                    Deps
                  </Button>
      ...
      <Dialog open={!!depsRow} onOpenChange={(v) => !v && setDepsRow(null)}>
        <DialogContent>
          ...
          {deps && deps.breakdown.length === 0 && (
            <div className="text-sm text-muted-foreground">No child references defined.</div>
          )}
          {deps && deps.breakdown.length > 0 && (
            <div className="space-y-2">
              ...
              {deps.hasLive && (
                <div className="flex gap-2 items-center text-amber-600 text-xs">
                  <AlertTriangle className="h-4 w-4" /> Hard delete blocked while live children
                  exist.
                </div>
              )}
            </div>
          ...
          {isOwner && depsRow && (
            <DialogFooter className="gap-2">
              <Button
                variant="outline"
                onClick={() => deleteM.mutate({ id: depsRow.id, force: false })}
              ...
              {deps?.hasLive && (
                <Button
                  variant="destructive"
                  onClick={() => {
                    if (confirm("Force delete will cascade orphan references. Continue?"))
                      deleteM.mutate({ id: depsRow.id, force: true });
                  }}
                >
                  Force delete
                </Button>
      ...
      <Dialog open={purgeOpen} onOpenChange={setPurgeOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Bulk purge archived rows</DialogTitle>
          </DialogHeader>
          <div className="text-sm text-muted-foreground space-y-2">
            <p>
              Permanently deletes archived rows older than the selected retention window, across
              every domain. Rows with live dependencies are skipped.
            </p>
          ...
          <DialogFooter className="gap-2">
            <Button
              variant="outline"
              onClick={() => purgeM.mutate(180)}
              disabled={purgeM.isPending}
            >
              180 days
            </Button>
            <Button variant="outline" onClick={() => purgeM.mutate(90)} disabled={purgeM.isPending}>
              90 days
            </Button>
            <Button
              variant="destructive"
              onClick={() => purgeM.mutate(30)}
              disabled={purgeM.isPending}
            >
              30 days
            </Button>
