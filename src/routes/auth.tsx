    ...
    try {
      if (mode === "signup") {
        ...
        if (!/^[A-Z0-9-]{4,16}$/.test(code))
          throw new Error("Invite codes are 4–16 letters/numbers.");
        ...
        if (error) {
          ...
          if (/email not confirmed|confirm/i.test(m)) {
            setNeedsVerify(email);
            throw new Error(
              "Email not verified yet. Check your inbox — we can resend the link below.",
            );
          }
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      <div className="hidden lg:flex surface-dark items-center justify-center p-12">
        <div className="max-w-md">
          <div className="font-display text-6xl text-[var(--color-gold)] leading-none tracking-[0.08em] uppercase">
            {branding.shortName}
          </div>
      ...
      <div className="flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          <div className="lg:hidden mb-6 flex flex-col items-center gap-2">
            <div className="font-display text-3xl text-foreground tracking-[0.08em] uppercase">
              {branding.shortName}
            </div>
          </div>
            ...
            {mode === "signup" && (
              <Field label="Invite code">
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  Get a code from your manager. Required to join.
                </div>
              </Field>
            )}
            <button
            ...
            {needsVerify && (
              <div className="mt-2 rounded-md border border-border bg-card p-3 text-xs">
                ...
                <div className="mt-1 text-muted-foreground">
                  ...
                  <span className="text-foreground">{needsVerify}</span>. Click it, then sign in.
                </div>
                <button
                  type="button"
...
function FullScreen({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen grid place-items-center text-muted-foreground">{children}</div>
  );
}
