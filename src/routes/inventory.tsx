...
import {
  ...
  listInventoryOrders,
  submitDraftInventoryOrder,
  decideInventoryOrder,
} from "@/lib/inventory-orders.functions";
...
function recommendedAction(s: Status): {
  label: string;
...
function InventoryPage() {
  ...
  const initial = useMemo<TabKey>(() => {
    if (search.tab && (TAB_KEYS as readonly string[]).includes(search.tab))
      return search.tab as TabKey;
  ...
  return (
    <AppShell>
      ...
      {tab === "orders" && (
        <OrdersTab
          onEditDetails={(itemId) => isManager && changeTab("configuration", { focus: itemId })}
        />
      )}
      {tab === "drafts" && isOwner && <DraftOrdersTab />}
      {tab === "approvals" && isManager && <InventoryChangesView />}
      {tab === "configuration" && isManager && (
        <OrderGuideView focusItemId={search.focus ?? null} />
      )}
    </AppShell>
  );
}
    ...
    <div>
      <SectionHeader
        ...
        action={
          <button
            onClick={() => refetch()}
            className="rounded-md border border-border px-2.5 py-1 text-xs font-semibold text-muted-foreground"
          >
            Refresh
          </button>
        }
      ...
      <p className="text-xs text-muted-foreground mb-3">
        Orders saved as drafts don't notify anyone. Submit them here to trigger the owner email +
        alert.
      </p>
      ...
      {isLoading ? (
        ...
        <Card className="p-6 text-center text-sm text-muted-foreground">
          No draft orders — you're all caught up.
        </Card>
      ) : (
        <div className="space-y-2">
          {orders.map((o: any) => {
            ...
            const critical = items.filter(
              (i: any) => i.urgency === "critical" || i.urgency === "emergency",
            ).length;
            return (
              <Card key={o.id} className="p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {o.notes && (
                      <div className="text-xs text-muted-foreground mt-1">Note: {o.notes}</div>
                    )}
                    {items.length > 0 && (
                      <div className="mt-2 text-xs text-muted-foreground line-clamp-2">
                        {items
                          .map(
                            (i: any) =>
                              `${i.item_name} (${i.requested_qty}${i.unit ? " " + i.unit : ""})`,
                          )
                          .join(" · ")}
                      </div>
                    )}
...
function LiveCountsTab() {
  ...
  const { data: items = [], isLoading } = useQuery<Item[]>({
    ...
    queryFn: () =>
      list({ data: { trailerId: trailerScope, includeArchived: showArchived } }) as Promise<Item[]>,
  ...
  function handleRemoveCategory() {
    ...
    if (!current) {
      toast.error("Pick a category tab first.");
      return;
    }
    if (
      !confirm(
        `Remove category "${current.label}"? Items in this category must be moved or archived first.`,
      )
    )
      return;
  ...
  const requestMut = useMutation({
    mutationFn: (vars: { item: Item; reason: string }) =>
      requestFn({
        data: {
          action: "archive",
          targetItemId: vars.item.id,
          trailerId: trailerScope,
          payload: { name: vars.item.name },
          reason: vars.reason,
        },
      }),
    onSuccess: () => {
      toast.success("Archive request sent to owner");
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-end gap-2">
        ...
        <div className="text-xs font-semibold uppercase tracking-[1.2px] text-[var(--color-gold)] bg-[#0A0A0A] px-3 py-1.5 rounded-md">
          {trailerLabel}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {visible.map((it) => {
          ...
          const pct = Math.min(
            150,
            Math.round((Number(it.current_qty) / Math.max(1, Number(it.par_level))) * 100),
          );
          return (
            <Card key={it.id} className={cn("p-3", it.archived_at && "opacity-60")}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="label-caps text-muted-foreground mt-0.5">
                    PAR {Number(it.par_level)} {it.unit}
                  </div>
                </div>
                    ...
                    {it.archived_at ? (
                      isOwner && (
                        <button
                          ...
                          className="rounded-md border border-border p-1.5 text-muted-foreground hover:text-[var(--color-success)] disabled:opacity-50"
                          title="Restore"
                        >
                      ...
                      <>
                        <button
                          ...
                          title={isOwner ? "Edit" : "Request edit"}
                        >
                          ...
                          onClick={() => {
                            if (isOwner) {
                              ...
                              const reason = window.prompt(
                                `Request to archive ${it.name}. Reason?`,
                              );
                              if (reason !== null) requestMut.mutate({ item: it, reason });
                            }
                          }}
                          disabled={deleteMut.isPending || archiveMut.isPending}
                          className="rounded-md border border-border p-1.5 text-muted-foreground hover:text-[var(--color-danger)] disabled:opacity-50"
                          title={isOwner ? "Archive or delete" : "Request archive"}
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </button>
                      </>
              ...
              <div className="mt-3 grid grid-cols-[1fr_auto_auto] gap-2 items-center">
                <div>
                  <div className="text-2xl font-semibold">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {it.unit} · {pct}%
                      {(it as any).cost_per_unit > 0
                        ? ` · $${Number((it as any).cost_per_unit).toFixed(2)}/${it.unit}`
                        : ""}
                    </span>
                  </div>
                ...
                <button
                  ...
                  className="rounded-md bg-[var(--color-gold)] text-[#0A0A0A] px-3 py-2 text-xs font-semibold inline-flex items-center gap-1"
                >
                  <Truck className="h-3.5 w-3.5" /> Receive
                </button>
                <button
                  onClick={() => setWasteItem(it)}
                  className="rounded-md border border-border px-3 py-2 text-xs font-semibold inline-flex items-center gap-1"
                >
      ...
      {editItem && (
        <EditItemModal
          ...
          onDone={() => syncDomains(qc, "inventory", "alerts")}
        />
      )}
...
function OrdersTab({ onEditDetails }: { onEditDetails: (itemId: string) => void }) {
  ...
  const statusToneFor = (s: string): "warning" | "success" | "info" | "neutral" =>
    s === "approved" ? "success" : s === "ordered" ? "info" : "warning";
      ...
      {!isLoading && pending.length === 0 && (
        <Card className="p-8 text-center">
          ...
          <div className="text-sm text-muted-foreground">
            No open orders waiting to be fulfilled.
          </div>
          <button
            onClick={() => {
              qc.invalidateQueries({ queryKey: ["inventory"] });
            ...
            <Card key={o.id} className="p-0 overflow-hidden">
              <div className="flex items-center justify-between gap-3 px-4 py-3 bg-secondary/40 border-b border-border flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      if (window.confirm(`Cancel this order (${items.length} items)?`))
                  ...
                  >
                    ...
                    {receiveMut.isPending && receiveMut.variables === o.id
                      ? "Fulfilling…"
                      : `Fulfill order (${items.length})`}
                  </button>
                </div>
              </div>
                ...
                {items.map((it: any) => (
                  <div key={it.id} className="flex items-center gap-3 px-4 py-2.5">
                    <button
                      ...
                      disabled={!it.item_id}
                      className={cn(
                        "min-w-0 flex-1 text-left text-sm font-semibold truncate",
                        it.item_id && "hover:text-[var(--color-gold)]",
                      )}
    ...
    <div className="rounded-md bg-secondary/40 px-3 py-2">
      ...
      <div
        className={cn(
          ...
          emphasis ? "text-base text-[var(--color-gold)]" : "text-sm text-foreground",
        )}
      >
        {value}
      </div>
  ...
  return (
    <div className="mt-3 flex items-center gap-2">
      ...
      <button
        ...
        className="h-9 px-3 rounded-md border border-border text-xs font-semibold disabled:opacity-40"
      >
        Submit count
      </button>
    </div>
...
function WasteModal({
  ...
  const [qty, setQty] = useState("");
  const [reason, setReason] = useState("Burned");
  const m = useMutation({
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl w-full max-w-md p-5 card-shadow"
...
function ModalActions({
  ...
  return (
    <div className="mt-5 flex items-center justify-end gap-2">
      <button onClick={onClose} className="rounded-md px-3 py-2 text-sm border border-border">
        Cancel
      </button>
      <button
        disabled={disabled}
...
export function EditItemModal({
  ...
  const [costPerUnit, setCostPerUnit] = useState<string>(
    it?.cost_per_unit != null ? String(it.cost_per_unit) : "",
  );
    ...
    <Modal title={title} onClose={onClose}>
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Low / critical alert ≤">
            <input
              type="number"
              value={low}
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Current quantity (optional)">
            <input
              type="number"
              value={qty}
              onChange={(e) => setQty(e.target.value)}
        ...
        <div className="border-t border-border pt-3">
          <div className="label-caps text-muted-foreground mb-2">
            Inventory Guide {isOwner ? "" : "(owner-only)"}
          </div>
      ...
      <ModalActions
        onClose={onClose}
        primary={isOwner ? (item ? "Save changes" : "Create item") : "Submit request"}
        disabled={!name.trim() || m.isPending || uploading}
        onSubmit={() => m.mutate()}
      />
    </Modal>
  );
...
function OrderBuilderModal({
  ...
  const m = useMutation({
    mutationFn: (submit: boolean) =>
      createOrder({ data: { trailerId, notes: notes || undefined, submit, items: rows } }) as any,
    onSuccess: (_d, submit) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl w-full max-w-3xl p-5 card-shadow max-h-[90vh] overflow-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
        ...
        <div className="space-y-3">
          <Field label="Add item from inventory">
            ...
            >
              ...
              {items
                .filter((i) => !rows.some((r) => r.itemId === i.id))
                .map((i) => (
                  <option key={i.id} value={i.id}>
                    {i.name} — {Number(i.current_qty)}/{Number(i.par_level)} {i.unit}
                  </option>
                ))}
            </select>
          </Field>
                  ...
                  {rows.map((row, i) => (
                    <tr key={i} className="border-t">
                      <td className="p-2 font-medium">
                        {row.itemName}
                        <div className="text-muted-foreground text-[10px]">{row.unit}</div>
                      </td>
                      ...
                      <td className="p-2">
                        <input
                          ...
                          onChange={(e) => updateRow(i, { requestedQty: Number(e.target.value) })}
                      ...
                      <td className="p-2">
                        <select
                          value={row.urgency}
                          onChange={(e) => updateRow(i, { urgency: e.target.value as any })}
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          ...
          <button
            disabled={rows.length === 0 || m.isPending}
...
function OrderHistoryModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl w-full max-w-2xl p-5 card-shadow max-h-[85vh] overflow-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {(o.items ?? []).length} items{o.notes ? ` · ${o.notes}` : ""}
                    </div>
                    {o.owner_comment && (
                      <div className="text-xs mt-1 italic">Owner: {o.owner_comment}</div>
                    )}
                  </div>
                  <StatusPill
                    tone={
...
function exportVarianceCSV(items: Item[], _trailerLabel: string) {
  const rows = items.map((it) => {
    ...
    const pct =
      it.par_level === 0
        ? ""
        : ((Number(it.current_qty) / Number(it.par_level)) * 100).toFixed(0) + "%";
    return [
...
function exportVariancePDF(
  items: Item[],
  trailerLabel: string,
  counts: { crit: number; low: number; ok: number },
) {
  const rows = items
    .slice()
    .sort((a, b) => {
    ...
    .map((it) => {
      ...
      const pct =
        it.par_level === 0
          ? "—"
          : Math.round((Number(it.current_qty) / Number(it.par_level)) * 100) + "%";
      return [
        CATEGORY_LABELS[it.category] ?? it.category,
        it.name,
