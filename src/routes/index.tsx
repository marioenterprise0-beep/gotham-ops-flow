...
import {
  ...
  SectionHeader,
  StatusPill,
} from "@/components/gotham/primitives";
import {
  Activity,
  AlertTriangle,
  BellRing,
...
function Dashboard() {
  ...
  if (loading) {
    return (
      <AppShell>
        <Card className="py-10 text-center text-sm text-muted-foreground">Loading dashboard…</Card>
      </AppShell>
    );
  }
    ...
    <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-center">
          <div>
            <div className="flex items-center gap-2 label-caps text-white/60">
              <span
                className={`h-1.5 w-1.5 rounded-full ${clockedIn ? "bg-[var(--color-success)] animate-pulse" : "bg-white/30"}`}
              />
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-white/70">
              <span className="flex items-center gap-1.5">
                <Timer className="h-3 w-3 text-[var(--color-gold)]" />
                {now.toLocaleString([], {
                  weekday: "short",
                  month: "short",
                  day: "numeric",
            ...
            <div className="mt-2 flex items-center gap-2">
              {clockedIn ? (
                <Link
                  to="/time-clock"
                  className="inline-flex items-center gap-1.5 rounded-md bg-white/10 text-white border border-white/15 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[1.2px] hover:bg-white/15"
                >
                  <LogOut className="h-3 w-3" /> Clock out · {elapsedLabel}
                </Link>
              ) : (
                <Link
                  to="/time-clock"
                  className="inline-flex items-center gap-1.5 rounded-md bg-[var(--color-gold)] text-[#0A0A0A] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[1.2px]"
                >
              ...
              <Link
                to="/my-tasks"
                className="inline-flex items-center gap-1.5 rounded-md bg-white/5 text-white border border-white/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[1.2px] hover:bg-white/10"
              >
      ...
      <div className="mt-3">
        <ShiftFlowTracker
          ...
          criticalAlertsOpen={
            (stats?.alerts?.pending ?? []).filter((a: any) => a.priority === "critical").length
          }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3">
        <DarkStat
          label="My Tasks Done"
          value={`${done}/${total || 0}`}
          sub={total === 0 ? "no tasks yet" : `${total - done} remaining`}
          tone={done === total && total > 0 ? "gold" : undefined}
        />
        <DarkStat
          label="Time on Shift"
          value={elapsedLabel}
          sub={clockedIn ? "since clock-in" : "not clocked in"}
        />
        <DarkStat
          label="Announcements"
          value={String(announcements.length)}
          sub={announcements.length ? "new for you" : "all caught up"}
          tone={announcements.length ? "gold" : undefined}
        />
        <DarkStat
          label="Shift Phase"
          value={(stats?.shift?.phase ?? "—").toString().toUpperCase()}
          sub={stats?.shift ? "active shift" : "no active shift"}
        />
      </div>
      ...
      <SectionHeader
        ...
        action={
          <Link
            to="/my-tasks"
            className="label-caps text-foreground/70 hover:text-[var(--color-gold)]"
          >
            Open list
          </Link>
        }
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {myTasks.slice(0, 8).map((t, i) => {
          ...
          return (
            ...
            >
              <button
                onClick={() => {
                  if (!isDone) completeM.mutate(t.id);
                }}
        ...
        {myTasks.length > 8 && (
          <Link
            to="/my-tasks"
            className="block border-t border-border p-3 text-center text-xs label-caps text-foreground/70 hover:text-[var(--color-gold)]"
          >
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {announcements.slice(0, 4).map((a, i) => (
          ...
          >
            ...
            <StatusPill
              tone={
                a.priority === "critical" ? "danger" : a.priority === "high" ? "warning" : "neutral"
              }
            >
              {a.priority ?? "normal"}
            </StatusPill>
...
function quickActionsForRole(role: RoleId | null, clockedIn: boolean) {
  const clock = {
    to: "/time-clock",
    label: clockedIn ? "Time Clock" : "Clock In",
    icon: clockedIn ? Clock : LogIn,
    primary: !clockedIn,
  };
...
function ManagerView({ stats, role }: { stats: any; role: any }) {
  ...
  const phaseLabel = stats?.shift?.phase
    ? String(stats.shift.phase).toUpperCase()
    : "NO ACTIVE SHIFT";
    ...
    <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-center">
          <div>
            <div className="flex items-center gap-2 label-caps text-[var(--color-gold)]/80">
              <span
                className={`h-1.5 w-1.5 rounded-full ${shiftActive ? "bg-[var(--color-success)] animate-pulse" : "bg-white/30"}`}
              />
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-white/70">
              ...
              <span className="flex items-center gap-1.5">
                <Users className="h-3 w-3 text-[var(--color-gold)]" />
                Crew: {crew.length}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {openedAt
                  ? `Opened ${openedAt.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true })}`
            ...
            {shiftActive ? (
              ...
              <Link
                to="/operations"
                className="inline-flex items-center gap-1.5 rounded-md bg-[var(--color-gold)] text-[#0A0A0A] px-3 py-1.5 text-xs font-semibold uppercase tracking-[1.2px]"
              >
        ...
        <Card>
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {clockedIn
                  ? `Since ${clockInAt!.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true })}`
            ...
            {clockedIn ? (
              <Link
                to="/time-clock"
                className="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-semibold uppercase tracking-[1.2px]"
              >
          ...
          <div className="mt-3">
            <Link
              to="/time-clock"
              className="text-xs label-caps text-foreground/70 hover:text-[var(--color-gold)] inline-flex items-center gap-1"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            ...
            <Card className="p-3">
              ...
              <div
                className={cn(
                  ...
                  stats.schedule.openShifts > 0 && "text-[var(--color-warning)]",
                )}
              >
            ...
            <Card className="p-3">
              ...
              <div className="font-display text-2xl mt-1">
                ${stats.schedule.laborCost.toLocaleString()}
              </div>
            </Card>
            <Card className="p-3">
              ...
              {stats.schedule.laborPct != null ? (
                <div
                  className={cn(
                    ...
                    stats.schedule.laborPct > 28 &&
                      stats.schedule.laborPct <= 35 &&
              ...
              {!stats.schedule.salesTarget && (
                <Link
                  to="/schedule"
                  className="text-[10px] text-muted-foreground hover:text-[var(--color-gold)]"
                >
                  set sales target
                </Link>
              )}
      ...
      <SectionHeader
        ...
        action={
          <Link
            ...
            className="label-caps text-foreground/70 hover:text-[var(--color-gold)]"
          >
            Open list ({myDone}/{myTasks.length})
          </Link>
        }
      />
      <Card className="p-0 overflow-hidden">
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {pending.slice(0, 5).map((a: any, i: number) => (
          <Link
            key={a.id}
            to="/alerts"
            className={cn("flex items-center justify-between p-4", i && "border-t border-border")}
          >
            ...
            <StatusPill tone={a.priority === "critical" ? "danger" : "warning"}>
              {a.priority ?? "normal"}
            </StatusPill>
          </Link>
        ))}
      ...
      <SectionHeader
        ...
        action={
          <Link
            to="/inventory"
            search={{ tab: undefined, focus: undefined }}
            className="label-caps text-foreground/70 hover:text-[var(--color-gold)]"
          >
            View inventory
          </Link>
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {lowStock.map((r: any, i: number) => (
          <div
            key={r.id}
            className={`flex items-center justify-between p-4 ${i ? "border-t border-border" : ""}`}
          >
            <div className="flex items-center gap-3 min-w-0">
            ...
            <StatusPill tone={r.critical ? "danger" : "warning"}>
              {r.critical ? "Critical" : "Low"} · {r.pct}%
            </StatusPill>
          </div>
        ))}
      </Card>
      ...
      <Card className="p-0 overflow-hidden">
        {pending.filter(
          (a: any) =>
            a.type === "approval_required" || a.type === "signoff" || a.type === "task_signoff",
        ).length === 0 && (
          <div className="p-6 text-center text-sm text-muted-foreground">
      ...
      <SectionHeader
        ...
        action={
          <div className="flex gap-2">
            <button
              onClick={() => exportHealthCSV(stats, crew, role?.name ?? "Crew")}
              className="inline-flex items-center gap-1 rounded-md border border-border px-2.5 py-1 text-xs font-semibold"
            >
              <Download className="h-3.5 w-3.5" /> CSV
            </button>
            <button
              onClick={() => exportHealthPDF(stats, crew, role?.name ?? "Crew", phaseLabel)}
              className="inline-flex items-center gap-1 rounded-md bg-[#0A0A0A] text-[var(--color-gold)] px-2.5 py-1 text-xs font-semibold"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <DarkStat
          ...
          tone={pending.length ? "gold" : undefined}
        />
      </div>
      ...
      <SectionHeader
        ...
        action={
          <Link
            ...
            className="label-caps text-foreground/70 hover:text-[var(--color-gold)]"
          >
            Open full analytics <ChevronRight className="inline h-3 w-3" />
          </Link>
        }
      />
        ...
        <Card className="hover:border-[var(--color-gold)] transition-colors">
          <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-5 items-center">
            <div className="flex items-center gap-4">
              <div
                className="grid place-items-center h-20 w-20 rounded-full border-4"
              ...
              <div>
                ...
                <StatusPill
                  tone={band === "green" ? "success" : band === "yellow" ? "warning" : "danger"}
                >
                  {band.toUpperCase()}
                </StatusPill>
              </div>
...
function DarkStat({
  ...
  return (
    <div className="rounded-xl border border-[#1C1C1C] bg-[#0A0A0A] p-4 relative overflow-hidden">
      ...
      <div
        className={`mt-1 text-3xl font-semibold ${tone === "gold" ? "text-[var(--color-gold)]" : tone === "danger" ? "text-[#FF6B5B]" : "text-white"}`}
      >
        {value}
      </div>
...
function ActionBtn({
  ...
  return (
    <Link
      to={to}
      className={`flex items-center gap-3 rounded-lg px-4 py-4 border transition-all ${
        primary
...
function exportHealthCSV(stats: any, crew: any[], roleName: string) {
  ...
  (stats?.alerts?.items ?? []).forEach((r: any) =>
    rows.push(["Stock", r.name, `${r.pct}% of par${r.critical ? " · CRITICAL" : ""}`]),
  );
...
function exportHealthPDF(stats: any, crew: any[], roleName: string, phaseLabel: string) {
  const items = (stats?.alerts?.items ?? []).map((r: any) => [
    r.name,
    `${r.pct}%`,
    r.critical ? "CRITICAL" : "LOW",
  ]);
  ...
  const html = `
    ...
    ${kpiBlock([
      {
        label: "Tasks Remaining",
        value: stats?.tasks?.remaining ?? 0,
        tone: (stats?.tasks?.remaining ?? 0) ? "warn" : "ok",
      },
      { label: "Tasks Done", value: `${stats?.tasks?.done ?? 0} / ${stats?.tasks?.total ?? 0}` },
      {
        label: "Alerts",
        value: stats?.alerts?.count ?? 0,
        tone: (stats?.alerts?.count ?? 0) ? "danger" : "ok",
      },
