    ...
    mutationFn: () =>
      insertLog({
        data: {
          entity: "system",
          action: "create",
          summary: "Change Log enabled",
          reason: "Module 9 rollout",
        },
      }),
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {grouped.map(([day, list], di) => (
          <div key={day}>
            <div
              className={cn(
                "px-4 py-2 text-xs font-semibold text-muted-foreground bg-secondary/40",
                di && "border-t border-border",
              )}
            >
              {day} · {list.length}
            </div>
            {list.map((r) => (
              ...
              >
                ...
                <div className="text-sm">
                  ...
                  {r.summary && (
                    <div className="text-xs text-muted-foreground mt-0.5 truncate">{r.summary}</div>
                  )}
                  {r.reason && (
      ...
      {selected && (
        ...
        >
          <div
            ...
            className="w-full max-w-2xl bg-card border border-border rounded-lg shadow-xl max-h-[85vh] overflow-auto"
          >
            <div className="sticky top-0 bg-card border-b border-border px-4 py-3 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold">
                  {selected.action.replace(/_/g, " ")} · {selected.entity}
                </div>
                <div className="text-xs text-muted-foreground">
                  {new Date(selected.created_at).toLocaleString()} · {selected.actor_name}
                </div>
              </div>
            ...
            <div className="p-4 space-y-4 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <pre className="bg-background border border-border rounded p-2 text-xs overflow-auto max-h-64">
