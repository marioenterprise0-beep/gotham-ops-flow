    ...
    <AppShell>
      <div className="flex items-end justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => setWeekStart(shiftWeek(weekStart, 7))}>
          ...
          <Button
            ...
            size="sm"
            disabled={!dash}
            onClick={() => exportLaborCSV(weekStart, dash)}
          >
          ...
          <Button
            ...
            size="sm"
            disabled={!dash}
            onClick={() => exportLaborPDF(weekStart, rangeLabel, dash)}
          >
      ...
      {dash && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
          ...
          <KPI
            ...
            value={String(dash.missedClockOuts)}
            tone={dash.missedClockOuts ? "warning" : "default"}
          />
          <KPI
            label="Pending corrections"
            value={String(dash.pendingCorrections)}
            tone={dash.pendingCorrections ? "warning" : "default"}
          />
          <KPI
            label="Pending time off"
            value={String(dash.pendingTimeOff)}
            tone={dash.pendingTimeOff ? "warning" : "default"}
          />
      ...
      <Tabs defaultValue="employees" className="mt-6">
        <TabsList>
          ...
          <TabsTrigger value="corrections">
            Corrections {dash?.pendingCorrections ? `(${dash.pendingCorrections})` : ""}
          </TabsTrigger>
          <TabsTrigger value="timeoff">
            Time Off {dash?.pendingTimeOff ? `(${dash.pendingTimeOff})` : ""}
          </TabsTrigger>
          <TabsTrigger value="availability">
            Availability {pendingAvail ? `(${pendingAvail})` : ""}
          </TabsTrigger>
        ...
        <TabsContent value="employees">
          <Card className="p-0 overflow-hidden">
            ...
            {(dash?.employees ?? []).map((e: any, i: number) => {
              const status =
                e.flags.includes("missed_clock_out") || e.flags.includes("no_show")
                  ? "danger"
                  : e.flags.includes("overtime")
                    ? "warning"
                    : "success";
              return (
                <button
                  ...
                  className={cn(
                    "w-full text-left p-3.5 flex items-center justify-between gap-3",
                    isOwner ? "hover:bg-secondary/50 cursor-pointer" : "cursor-default",
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Scheduled {fmtH(e.scheduledMin)} · Worked {fmtH(e.workedMin)} · Diff{" "}
                      {fmtH(e.diffMin)}
                    </div>
                    {e.flags.length > 0 && (
    ...
    <Card className="p-3">
      ...
      <div
        className={cn(
          ...
          tone === "warning" ? "text-[var(--color-danger)]" : "text-foreground",
        )}
      >
        {value}
      </div>
...
function EmployeeDrawer({
  ...
  const { data } = useQuery({
    queryKey: ["emp-week", userId, weekStart],
    queryFn: () => fn({ data: { userId, weekStart } }),
  });
  const [editing, setEditing] = useState<string | null>(null);
  const [draft, setDraft] = useState<{ in: string; out: string; brk: number; reason: string }>({
    in: "",
    out: "",
    brk: 0,
    reason: "",
  ...
  for (const p of punches) {
    const key = new Date(p.clock_in_at).toLocaleDateString(undefined, {
      weekday: "short",
      month: "short",
      day: "numeric",
    });
  ...
  const auditFlags = (p: any): { label: string; tone: "success" | "warning" | "danger" }[] => {
    ...
    if (p.status === "auto_closed")
      flags.push({ label: "Auto-closed at scheduled end", tone: "warning" });
    if (p.status === "edited") flags.push({ label: "Owner edited", tone: "success" });
    if (typeof p.notes === "string" && /grace/i.test(p.notes))
      flags.push({ label: "Grace applied", tone: "warning" });
    return flags;
  };
  ...
  return (
    <Card className="mt-4 p-4">
      ...
      {punches.length === 0 && (
        <div className="text-sm text-muted-foreground py-3">No punches this week.</div>
      )}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 text-xs space-y-1">
                          <div className="font-mono">
                            ...
                            {p.clock_out_at
                              ? new Date(p.clock_out_at).toLocaleTimeString([], {
                                  hour: "numeric",
                                  minute: "2-digit",
                                })
                              : "—"}
                          ...
                          <div className="flex flex-wrap gap-1 items-center">
                            ...
                            {p.edited_at && (
                              <span className="text-[10px] text-muted-foreground">
                                edited {new Date(p.edited_at).toLocaleString()}
                              </span>
                            )}
                          </div>
                          {p.notes && (
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setDraft({
                                ...
                                out: p.clock_out_at
                                  ? new Date(p.clock_out_at).toISOString().slice(0, 16)
                                  : "",
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <Input
                            type="number"
                            value={draft.brk}
                            onChange={(e) => setDraft({ ...draft, brk: Number(e.target.value) })}
                          />
                        </div>
                        <div>
                          <Label>Reason</Label>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="min-w-0 space-y-0.5">
          <div className="text-sm font-semibold">
            {c.employee_name} · {c.type.replace(/_/g, " ")}
          </div>
          <div className="text-xs text-muted-foreground">
            For {c.for_date} · {c.reason}
          </div>
          {cp && (
            <div className="text-xs text-muted-foreground">
              On record: {new Date(cp.clock_in_at).toLocaleString()} →{" "}
              {cp.clock_out_at ? (
                new Date(cp.clock_out_at).toLocaleString()
              ) : (
                <span className="text-[var(--color-warning)]">open</span>
              )}{" "}
          ...
          {(c.requested_in || c.requested_out) && (
            <div className="text-xs font-medium text-[var(--color-gold)]">
              Requested: {c.requested_in ? new Date(c.requested_in).toLocaleString() : "—"} →{" "}
              {c.requested_out ? new Date(c.requested_out).toLocaleString() : "—"}
            </div>
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <StatusPill
            tone={
              c.status === "approved" ? "success" : c.status === "declined" ? "danger" : "warning"
            }
          >
            {c.status}
          </StatusPill>
          ...
          {c.status === "pending" && !isOwner && (
            <StatusPill tone="warning">Owner approval required</StatusPill>
          )}
        </div>
      </div>
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 rounded-md border border-border bg-secondary/30 p-3">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Reason for your adjustment"
            />
          </div>
          <div className="sm:col-span-2 flex gap-2 flex-wrap">
            <Button size="sm" onClick={() => m.mutate("approved")} disabled={m.isPending}>
              <Check className="h-3.5 w-3.5 mr-1" /> Approve with these times
            </Button>
            <Button
              size="sm"
              variant="outline"
...
function CorrectionsList({ items, isOwner }: { items: any[]; isOwner: boolean }) {
  if (items.length === 0)
    return (
      <Card className="mt-3 p-6 text-center text-sm text-muted-foreground">No corrections.</Card>
    );
  return (
      ...
      {items.map((t, i) => (
        <div key={t.id} className={cn("p-3.5", i && "border-t border-border")}>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="min-w-0">
              <div className="text-sm font-semibold">
                {t.employee_name} · {t.start_date} → {t.end_date}
              </div>
              <div className="text-xs text-muted-foreground">
                {t.full_day
                  ? "Full day(s)"
                  : `${fmtTime12(t.start_time)} – ${fmtTime12(t.end_time)}`}{" "}
                · {t.reason}
              </div>
            </div>
            <div className="flex items-center gap-2">
...
function AvailabilityDecisionButtons({ id }: { id: string }) {
  ...
  const m = useMutation({
    mutationFn: (decision: "approved" | "declined") =>
      fn({ data: { id, decision, note: note || undefined } }),
    onSuccess: () => {
      toast.success("Decision recorded");
      setNote("");
      setShowNote(false);
...
function NotesList({ items }: { items: any[] }) {
  if (items.length === 0)
    return <Card className="mt-3 p-6 text-center text-sm text-muted-foreground">No notes.</Card>;
  return (
    <Card className="mt-3 p-0 overflow-hidden">
      {items.map((n, i) => (
...
function exportPayrollCSV(weekStart: string, detail: any) {
  ...
  for (const emp of detail.employees ?? []) {
    ...
    for (const p of emp.punches ?? []) {
      ...
      const diffMin =
        (new Date(p.clock_out_at).getTime() - new Date(p.clock_in_at).getTime()) / 60000;
...
function exportLaborPDF(weekStart: string, rangeLabel: string, dash: any) {
  ...
  const html = `
    ...
    ${kpiBlock([
      ...
      {
        label: "Difference",
        value: ((dash.totals.worked - dash.totals.scheduled) / 60).toFixed(1) + "h",
      },
      { label: "Open Shifts", value: dash.openShifts ?? 0 },
      {
        label: "Missed Clock-outs",
        value: dash.missedClockOuts ?? 0,
        tone: dash.missedClockOuts ? "danger" : "ok",
      },
      {
        label: "Pending Corrections",
        value: dash.pendingCorrections ?? 0,
        tone: dash.pendingCorrections ? "warn" : "ok",
      },
      {
        label: "Pending Time Off",
        value: dash.pendingTimeOff ?? 0,
        tone: dash.pendingTimeOff ? "warn" : "ok",
      },
