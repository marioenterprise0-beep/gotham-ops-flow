...
function RecapsPage() {
  ...
  const myToday = useMemo(
    () =>
      recaps.find(
        (r) => r.manager_id === userId && r.recap_date === new Date().toISOString().slice(0, 10),
      ),
    [recaps, userId],
  );
      ...
      <div className="space-y-2">
        {recaps.length === 0 && (
          <EmptyState
            ...
            title={
              tab === "today"
                ? "No recap for today yet"
                : tab === "pending"
                  ? "No recaps awaiting review"
                  : "No historical recaps"
            }
            hint={
              tab === "today"
                ? "Capture the shift while it's fresh — your team will thank you."
                : tab === "pending"
                  ? "Submitted recaps will land here for owner sign-off."
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="font-semibold text-sm">
                  {r.manager_name} ·{" "}
                  {trailers.find((t) => t.id === r.trailer_id)?.name ?? r.location ?? "—"}
                </div>
                  ...
                  {r.shift_score != null && (
                    <>
                      ...
                      <span className="inline-flex items-center gap-0.5">
                        <Star className="h-3 w-3" /> {r.shift_score}/10
                      </span>
                    </>
                  )}
                </div>
...
const SECTIONS: Array<{ heading: string; fields: Array<{ key: string; label: string }> }> = [
  ...
  {
    heading: "Next Shift Notes",
    fields: [{ key: "nextShiftNotes", label: "Anything next crew should know" }],
  },
];

const FIELD_TO_DB: Record<string, string> = {
  opsWentWell: "ops_went_well",
  opsSlowed: "ops_slowed",
  opsAttention: "ops_attention",
  invLowStock: "inv_low_stock",
  invConcerns: "inv_concerns",
  invOrders: "inv_orders",
  laborAttendance: "labor_attendance",
  laborStaffing: "labor_staffing",
  laborPerformance: "labor_performance",
  hospFeedback: "hosp_feedback",
  hospWins: "hosp_wins",
  hospComplaints: "hosp_complaints",
...
function RecapEditor({
  ...
  const reviewM = useMutation({
    mutationFn: (action: "review" | "archive") =>
      review({ data: { id: existing!.id, action, comment: ownerComment || undefined } }) as any,
    onSuccess: (_d, action) => {
      toast.success(action === "archive" ? "Archived" : "Marked reviewed");
      onSaved();
      onClose();
    },
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl w-full max-w-3xl p-5 card-shadow max-h-[92vh] overflow-auto"
      ...
      >
          ...
          <div>
            ...
            {existing && (
              <div className="text-xs text-muted-foreground mt-0.5">
                {existing.manager_name} · {existing.recap_date} · {status}
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <Field label="Location">
                <select
                  disabled={!editable}
                  value={trailerId ?? ""}
                  onChange={(e) => setTrailerId(e.target.value || null)}
              ...
              <Field label="Shift Score (1–10)">
                <input
                  ...
                  value={shiftScore}
                  onChange={(e) =>
                    setShiftScore(e.target.value === "" ? "" : Number(e.target.value))
                  }
                  className="w-full h-10 rounded-md border border-border bg-card px-3 text-sm"
                />
              </Field>
        ...
        <div className="mt-5 flex items-center justify-end gap-2 flex-wrap">
          ...
          {editable && (
            <>
              <button
                ...
                className="rounded-md px-3 py-2 text-sm border border-border disabled:opacity-50"
              >
                Save Draft
              </button>
              <button
          ...
          {isOwner && existing && status === "submitted" && (
            <>
              <button
                ...
                onClick={() => reviewM.mutate("review")}
                className="rounded-md px-3 py-2 text-sm font-semibold bg-[var(--color-success)] text-white inline-flex items-center gap-2 disabled:opacity-50"
              >
          ...
          {isOwner && existing && (status === "reviewed" || status === "submitted") && (
            <button
              disabled={reviewM.isPending}
              onClick={() => reviewM.mutate("archive")}
              className="rounded-md px-3 py-2 text-sm border border-border inline-flex items-center gap-2 disabled:opacity-50"
            >
