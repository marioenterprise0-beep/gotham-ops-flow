    ...
    <AppShell>
      ...
      <Card dark>
        <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-6 items-center">
          <div className="text-center md:text-left">
            <div className="label-caps text-white/55">
              {view === "today" ? "Today's Hospitality Score" : `${monthLabel(month)} Score`}
            </div>
              ...
              breakdown.map((b: any) => (
                <div key={b.key}>
                  <div className="flex justify-between text-xs mb-1.5 text-white/80">
                    <span>
                      {b.label} {b.count > 0 && <span className="text-white/50">({b.count})</span>}
                    </span>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button
          ...
          className="rounded-lg bg-[var(--color-gold)] text-[#0A0A0A] font-semibold text-sm px-4 py-3 inline-flex items-center justify-center gap-2"
        >
          <Plus className="h-4 w-4" /> Log Observation
        </button>
        <button
          onClick={() => setShowRec(true)}
          className="rounded-lg border border-border bg-card font-semibold text-sm px-4 py-3 inline-flex items-center justify-center gap-2 hover:border-[var(--color-gold)]"
        >
      ...
      <SectionHeader
        eyebrow={view === "today" ? "Today" : monthLabel(month)}
        title="Guest Recovery Log"
        action={<StatusPill tone="neutral">{recoveries.length} recoveries</StatusPill>}
      />
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {recoveries.map((r: any, i: number) => (
          <div
            key={r.id}
            className={cn(
              "grid grid-cols-1 md:grid-cols-[80px_120px_1.4fr_1.4fr_120px] gap-3 px-4 py-3 text-sm",
          ...
          >
            ...
            <div>
              <StatusPill
                tone={
                  r.severity === "high" ? "danger" : r.severity === "medium" ? "gold" : "success"
                }
              >
                {r.severity}
              </StatusPill>
      ...
      {showLog && (
        <LogModal kind="observation" onClose={() => setShowLog(false)} onSaved={onSaved} />
      )}
...
function LogModal({
  ...
  const mut = useMutation({
    ...
    onSuccess: () => {
      toast.success(kind === "recovery" ? "Recovery logged" : "Observation logged");
      onSaved();
      onClose();
    },
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display text-xl">
            {kind === "recovery" ? "LOG RECOVERY" : "LOG OBSERVATION"}
          </h3>
        ...
        <label className="block mb-3">
          <div className="label-caps text-muted-foreground mb-1">
            {kind === "recovery" ? "Issue description" : "Notes"}
          </div>
          <input
            value={notes}
        ...
        <div className="text-xs text-muted-foreground">
          Timestamp auto-recorded at save. Counts feed the hospitality score and analytics.
        </div>
