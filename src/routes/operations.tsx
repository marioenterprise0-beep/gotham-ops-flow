...
function Operations() {
  ...
  const openM = useMutation({
    ...
    onSuccess: () => {
      ...
      syncDomains(qc, "operations", "tasks", "labor");
    },
  ...
  const closeM = useMutation({
    ...
    onSuccess: () => {
      ...
      syncDomains(qc, "operations", "tasks", "labor", "recaps");
    },
  });
      ...
      <Card dark>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h1 className="font-display text-3xl mt-1 text-white">
              {phase.toUpperCase()} CHECKLIST
            </h1>
            <div className="mt-1 text-xs text-white/60 flex items-center gap-2">
              <Timer className="h-3.5 w-3.5" /> Opened{" "}
            ...
            <Card>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <label className="flex items-center gap-2 text-xs text-muted-foreground px-2 border border-border rounded-md">
                    <input
                      type="checkbox"
                      checked={newSignoff}
                      onChange={(e) => setNewSignoff(e.target.checked)}
                    />
                    Sign-off
                  </label>
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button
                    onClick={() => addTaskM.mutate()}
                    disabled={!newTitle.trim() || addTaskM.isPending}
        ...
        return (
          <div key={section}>
            <div className="mt-5 rounded-lg surface-dark px-4 py-3 flex items-center gap-3">
              <span className="font-display text-[var(--color-gold)] tracking-wider">
                {section}
              </span>
              <span className="label-caps text-white/55">
                {sDone}/{list.length} complete
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    {isDone && !isSigned && t.requires_signoff && isManager && (
                      <button
                        onClick={() => signOffM.mutate({ taskId: t.id, approve: true })}
                    ...
                    {isOwner && (
                      <button
                        onClick={() => {
                          if (
                            confirm(
                              `Delete task "${t.title}"? This cannot be undone from the checklist.`,
                            )
                          ) {
                            deleteM.mutate(t.id);
                          }
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-0.5">
            Target {TARGETS[phase]} · {new Date().toLocaleDateString()}
          </div>
        </div>
        <button
          onClick={() => {
