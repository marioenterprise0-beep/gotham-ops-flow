...
export const Route = createFileRoute("/audit")({
  ssr: false,
  beforeLoad: () => {
    throw redirect({ to: "/admin", search: { tab: "activity" } as any });
  },
...
function toneFor(action: string): "success" | "warning" | "danger" | "info" | "gold" {
  ...
  if (action.endsWith("_purged") || action.endsWith("_deleted") || action.includes("purge"))
    return "danger";
  return "info";
}
    ...
    <EmbedShell>
      ...
      {!isLoading && rows.length === 0 && (
        <EmptyState
          ...
          hint="Every meaningful action — punches, approvals, edits — gets logged here for review."
        />
      )}
      {(isLoading || rows.length > 0) && (
        <Card className="p-0 overflow-hidden">
          ...
          {rows.map((r, i) => {
            ...
            return (
              <button
                key={r.id}
                onClick={() => setPayloadView(r)}
                className={cn(
                  "w-full text-left p-4 grid grid-cols-1 md:grid-cols-[160px_160px_1fr_auto] gap-3 items-start hover:bg-muted/40",
              ...
              >
                ...
                <div className="text-sm min-w-0">
                  ...
                  {payloadStr && (
                    <div className="text-xs text-muted-foreground mt-1 truncate">{payloadStr}</div>
                  )}
                </div>
      ...
      <Dialog open={!!payloadView} onOpenChange={(v) => !v && setPayloadView(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {payloadView?.action} · {payloadView?.entity}
            </DialogTitle>
          </DialogHeader>
          {payloadView && (
            <div className="space-y-2 text-sm">
              <div>
                ...
                {new Date(payloadView.created_at).toLocaleString()}
              </div>
              <div>
                <span className="text-muted-foreground">Actor:</span> {payloadView.actor_name}{" "}
                {payloadView.actor_email && (
                  <span className="text-xs text-muted-foreground">({payloadView.actor_email})</span>
                )}
              </div>
              <div>
                <span className="text-muted-foreground">Entity ID:</span>{" "}
                <code className="text-xs">{payloadView.entity_id ?? "—"}</code>
              </div>
              <div>
                <div className="text-muted-foreground mb-1">Payload</div>
                <pre className="text-xs bg-muted p-3 rounded-md overflow-x-auto max-h-[50vh]">
