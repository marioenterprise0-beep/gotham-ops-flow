      ...
      for (const l of lines) {
        ...
        if (colonIdx > 0 && colonIdx < 40) {
          out.steps.push({
            ...
            instruction: text.slice(colonIdx + 1).trim(),
          });
        } else {
...
const TRAINING_CATEGORIES = [
  {
    ...
    label: "Operations",
    icon: ClipboardList,
    blurb: "Opening, closing, cash, inventory.",
  ...
  },
  {
    ...
    label: "Product Build",
    icon: Utensils,
    blurb: "Grill, prep, recipe execution.",
  ...
  },
  {
    ...
    label: "Hospitality",
    icon: Heart,
    blurb: "Guest service, cashier, front line.",
  ...
  },
  {
    ...
    label: "Cleaning",
    icon: Brush,
    blurb: "Sanitation, deep clean, station resets.",
    match: ["Cleaning"],
  },
  {
    key: "Emergency",
    label: "Emergency",
    icon: Siren,
    blurb: "Food safety, incidents, escalation.",
...
function SOPsPage() {
  ...
  const myAckMap = useMemo(() => {
    ...
    for (const a of myAcks as any[])
      m.set(a.sop_id, { version: a.version, acknowledged_at: a.acknowledged_at });
    return m;
  }, [myAcks]);
  ...
  const filtered = useMemo(() => {
    ...
    return (sops as any[]).filter((s) => {
      ...
      if (!term) return true;
      return [s.title, s.body, s.category, s.role].some((v) =>
  ...
  const activeSop = useMemo(
    () => (sops as any[]).find((s) => s.id === openId) ?? null,
    [sops, openId],
  );
    ...
    <AppShell>
      ...
      {!filterActive && continueSop && (
        <div className="mt-5">
          ...
          <ContinueCard
            sop={continueSop}
            myAck={myAckMap.get(continueSop.id) ?? null}
            onResume={() => setOpenId(continueSop.id)}
          />
        </div>
      )}
      ...
      {!filterActive && (
        <div className="mt-6">
          ...
          {assigned.length === 0 ? (
            <EmptyState
              icon={GraduationCap}
              title="Assign first SOP"
              hint="Owners can publish SOPs and assign them to your role — they'll appear here as required training."
            />
          ) : (
                ...
                >
                  ...
                  <div className="mt-3 flex items-center gap-3 text-xs">
                    <span className="text-muted-foreground">
                      {c.total} {c.total === 1 ? "SOP" : "SOPs"}
                    </span>
                    {c.remaining > 0 ? (
                      <span className="text-[var(--color-gold)] font-semibold">
                        {c.remaining} to complete
                      </span>
                    ) : c.total > 0 ? (
      ...
      {!filterActive && (
        <div className="mt-6">
          <SectionHeader
            eyebrow="Your record"
            title={`Completed certifications · ${completed.length}`}
          />
          {completed.length === 0 ? (
            <EmptyState
              icon={Award}
              title="No certifications yet"
              hint="Finish an SOP end-to-end to earn your first certification badge."
            />
          ) : (
      ...
      {filterActive && (
        <div className="mt-5">
          <div className="flex items-center justify-between gap-2 mb-3">
            <SectionHeader
              eyebrow={activeBucket === "All" ? "Search results" : activeBucket}
              title={
                isLoading
                  ? "Loading…"
          ...
          {filtered.length === 0 ? (
            <Card>
              <p className="text-sm text-muted-foreground">No procedures match.</p>
            </Card>
          ) : (
            <div className="space-y-2">
              {filtered.map((s) => (
...
function ContinueCard({
  ...
  sop: any;
  myAck: { version: number } | null;
  onResume: () => void;
}) {
  const parsed = useMemo(
    () => parseSopBody(sop.body ?? "", sop.pass_standard),
    [sop.body, sop.pass_standard],
  );
...
function CertRow({
  ...
  sop: any;
  myAck: { version: number; acknowledged_at: string };
  onOpen: () => void;
}) {
  return (
    <button
...
function SopDetail({
    ...
    return [
      {
        ...
        instruction: parsed.freeform ?? sop.body ?? "",
  ...
  const toggleDone = (i: number) =>
    setRunState((rs) => rs.map((r, idx) => (idx === i ? { ...r, done: !r.done } : r)));
  const updateNote = (i: number, v: string) =>
    setRunState((rs) => rs.map((r, idx) => (idx === i ? { ...r, note: v } : r)));
      ...
      <div className="pb-28 lg:pb-6">
        ...
        <Card dark>
          ...
          <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <MetaCell
              label="Status"
              value={ackCurrent ? "Certified" : ackStale ? "Re-ack needed" : "In progress"}
          ...
          {ackStale && (
            <div className="mt-3 rounded-md border border-[var(--color-warning,#C9973A)]/40 bg-[var(--color-warning,#C9973A)]/10 px-3 py-2 text-xs text-white/80">
              SOP updated since your last sign-off (you acknowledged v{myAck?.version}). Please
              review and re-acknowledge.
            </div>
          )}
        </Card>
            ...
            <Card>
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  {runState[active]?.done ? (
                    ...
                    <>
                      <CircleDot className="h-3.5 w-3.5" /> Mark complete
                    </>
                  )}
                </button>
              </div>
              ...
              {step.standard && (
                <div className="mt-4 rounded-md border border-[var(--color-success)]/30 bg-[var(--color-success-bg)] px-3 py-2 text-sm">
                  <div className="label-caps text-[var(--color-success)] mb-1 flex items-center gap-1.5">
                    <Target className="h-3.5 w-3.5" /> Standard
                  </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => {
                    if (!runState[active]?.done) toggleDone(active);
            ...
            <Card>
              ...
              {attachments.length === 0 ? (
                <p className="text-xs text-muted-foreground">
                  No reference media for this SOP yet.
                </p>
              ) : (
              ...
              <div className="flex flex-col gap-2 justify-center">
                ...
                {!allStepsDone && !ackCurrent && (
                  <p className="text-[11px] text-muted-foreground text-center">
                    Finish all steps to enable sign-off.
                  </p>
                )}
                {canSignOff && (
                  <div className="mt-1 rounded-md border border-border bg-secondary px-3 py-2 text-[11px] text-muted-foreground">
                    ...
                    Owners and managers verify completion from the SOP rollup on the training
                    dashboard.
                  </div>
                )}
              </div>
...
function MetaCell({
  ...
  label: string;
  value: string;
  accent?: "success" | "warning" | "gold";
}) {
  const color =
    accent === "success"
      ? "text-[var(--color-success)]"
      : accent === "warning"
        ? "text-[var(--color-warning,#C9973A)]"
        : accent === "gold"
          ? "text-[var(--color-gold)]"
          : "text-white";
  return (
...
function SopEditorModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-stretch sm:items-center justify-center bg-black/70 p-0 sm:p-4 overflow-y-auto"
      onClick={onClose}
    >
      <div
        className="w-full sm:max-w-2xl rounded-none sm:rounded-xl border border-border bg-card p-5 my-0 sm:my-8"
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                Use <code className="bg-muted px-1">## Purpose</code>,{" "}
                <code className="bg-muted px-1">## Steps</code> (numbered),
                <code className="bg-muted px-1">## Pass standard</code>,{" "}
                <code className="bg-muted px-1">## Common mistakes</code>, etc. Steps render best as{" "}
                <code className="bg-muted px-1">1. Title: Instruction</code>.
              </p>
            </div>
            <input
              value={form.passStandard}
              onChange={(e) => setForm({ ...form, passStandard: e.target.value })}
            ...
            {!depReport ? (
              ...
              <>
                <p className="mt-2 text-sm text-muted-foreground">
                  No history references. Safe to delete permanently.
                </p>
              ...
              <>
                <p className="mt-2 text-sm">
                  {depReport.totalRefs} historical reference{depReport.totalRefs === 1 ? "" : "s"}{" "}
                  found. Archive to preserve them.
                </p>
    ...
    <div>
      ...
      {!isLoading && versions.length === 0 && (
        <div className="text-xs text-muted-foreground">
          No previous versions yet — edits will be recorded here.
        </div>
      )}
      <div className="space-y-2 max-h-[60vh] overflow-auto">
        {versions.map((v) => (
          <div key={v.id} className="rounded-md border border-border p-3">
            ...
            <div className="mt-2 text-[12px] text-muted-foreground whitespace-pre-wrap line-clamp-6">
              {v.body}
            </div>
          </div>
        ))}
...
function SopAttachDrawer({ sop, canEdit }: { sop: any; canEdit: boolean }) {
  ...
  const onUpload = async (file: File) => {
    ...
    try {
      ...
      await addFn({
        data: { sopId: sop.id, storagePath: path, label: file.name, contentType: file.type },
      });
        ...
        {!isLoading && rows.length > 0 && (
          <>
            <div className="grid grid-cols-3 gap-2 mb-3">
              ...
              <Stat
                label="Current sign-offs"
                value={`${totals.ack}/${totals.slots}`}
                accent="success"
              />
              <Stat
                label="Stale (re-ack needed)"
                value={totals.stale}
                accent={totals.stale > 0 ? "warn" : undefined}
              />
            </div>
            <div className="space-y-2">
              {rows.map((r) => {
                const pct =
                  r.total_users > 0 ? Math.round((r.ack_current / r.total_users) * 100) : 0;
