...
export function StatusPill({ tone = "neutral", children }: { tone?: Tone; children: ReactNode }) {
  const map: Record<Tone, string> = {
    ...
    warning: "bg-[var(--color-warning-bg)] text-[var(--color-warning)] border border-[var(--color-warning)]/40",
    danger:  "bg-[var(--color-danger-bg)] text-[var(--color-danger)] border border-[var(--color-danger)]/40",
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 label-caps", map[tone])}>
...
export function ProgressBar({ value, tone = "gold" }: { value: number; tone?: "gold" | "success" | "danger" }) {
  const color = tone === "gold" ? "bg-[var(--color-gold)]" : tone === "success" ? "bg-[var(--color-success)]" : "bg-[var(--color-danger)]";
  return (
...
export function CircularProgress({ value, size = 72, stroke = 6, label }: { value: number; size?: number; stroke?: number; label?: string }) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle cx={size/2} cy={size/2} r={r} stroke="#C9973A" strokeWidth={stroke} fill="none"
...
export function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[1.2px]"
