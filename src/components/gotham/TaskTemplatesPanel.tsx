...
} from "@/lib/task-templates.functions";
    ...
    <Card className="p-0 overflow-hidden">
        ...
        <div className="ml-auto">
          {canEdit ? (
            ...
            <span className="label-caps text-muted-foreground">
              Read-only · Owner manages master templates
            </span>
          )}
        </div>
      </div>
      ...
      {filtered.map((t, i) => (
        <div
          key={t.id}
          className={cn(
            "grid grid-cols-1 md:grid-cols-[1.6fr_120px_110px_140px_90px_120px] gap-3 px-4 py-3 text-sm items-center",
        ...
        >
          ...
          <div className="uppercase text-xs tracking-wider">
            {ROLE_OPTS.find((r) => r.id === t.role)?.label ?? t.role}
          </div>
          ...
          <div className="flex gap-2 md:justify-end">
            <button
              ...
              className="rounded-md border border-border px-2.5 py-1.5 text-xs font-semibold inline-flex items-center gap-1"
            >
            ...
            {canEdit && (
              <>
                ...
                <button
                  onClick={() => {
                    if (confirm(`Delete "${t.title}"? This stops auto-assigning it.`))
      ...
      {historyFor && <HistoryModal template={historyFor} onClose={() => setHistoryFor(null)} />}
    </Card>
  );
}
...
function TemplateModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl w-full max-w-lg p-5 card-shadow"
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <select
                value={d.phase}
                onChange={(e) => setD({ ...d, phase: e.target.value as Phase })}
                className="w-full h-10 rounded-md border border-border bg-card px-2 text-sm"
              >
                {PHASE_OPTS.map((p) => (
                  <option key={p} value={p}>
            ...
            <div className="col-span-2">
              ...
              <select
                value={d.trailer_id ?? ""}
                onChange={(e) => setD({ ...d, trailer_id: e.target.value || null })}
                className="w-full h-10 rounded-md border border-border bg-card px-2 text-sm"
              >
            ...
            <div className="flex items-end gap-4">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={d.requires_signoff}
                  onChange={(e) => setD({ ...d, requires_signoff: e.target.checked })}
                />
                Sign-off
              </label>
              <label className="flex items-center gap-2 text-sm">
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            onClick={() =>
              onSave({ ...d, title: d.title.trim(), description: d.description?.trim() || null })
            }
...
function HistoryModal({ template, onClose }: { template: Template; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-xl w-full max-w-2xl max-h-[85vh] flex flex-col card-shadow"
        onClick={(e) => e.stopPropagation()}
      >
        ...
        <div className="overflow-y-auto p-5 space-y-4">
          {isLoading && (
            <div className="text-center text-sm text-muted-foreground py-6">Loading history…</div>
          )}
            ...
            return (
              <div key={v.id} className="rounded-md border border-border overflow-hidden">
                ...
                <div className="text-xs text-muted-foreground px-3 pt-2">
                  Applied to shifts opened after{" "}
                  {ts.toLocaleString([], {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                ...
                <div className="p-3 space-y-1.5">
                  {v.action === "create" && (
                    <div className="text-xs">
                      Created with title <span className="font-semibold">{v.after?.title}</span>,
                      role <span className="font-semibold">{v.after?.role}</span>, phase{" "}
                      <span className="font-semibold">{v.after?.phase}</span>.
                    </div>
                  )}
                  {v.action === "delete" && (
                    <div className="text-xs">
                      Template deleted. Stopped auto-assigning at the time above.
                    </div>
                  )}
                  {v.action === "update" &&
