...
export const listAlerts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let itemsQ = supabase
      ...
      .select("id,name,current_qty,low_threshold,par_level,trailer_id")
      .is("archived_at", null);
    ...
    for (const it of items ?? []) {
      const cur = Number(it.current_qty),
        low = Number(it.low_threshold),
        par = Number(it.par_level || 0);
      if (cur <= low && low > 0) {
        synthetic.push({
          id: `stock-${it.id}`,
          type: "critical_stock",
    ...
    if (data?.category) {
      if (data.category === "announcements") {
        return combined.filter(
          (a: any) => a.source_module === "announcements" || a.type === "announcement",
        );
      }
...
export const actOnAlert = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: alert, error: ge } = await supabase
      .from("alerts")
      .select("*")
      .eq("id", data.alertId)
      .single();
    ...
    if (data.action === "approve") patch = { status: "approved" };
      ...
      data.action === "resolve" ||
      data.action === "mark_received" ||
      data.action === "mark_ordered"
    ) {
      patch = {
        status: "resolved",
        resolved_by: userId,
        resolved_at: new Date().toISOString(),
        resolution: data.note ?? data.action,
      };
      ...
      patch = {
        assigned_role: "owner",
        priority: alert.priority === "low" ? "normal" : alert.priority,
      };
    }
    ...
    if (alert.type === "inventory_order" && alert.source_id) {
      ...
      if (map[data.action]) {
        ...
        if (
          data.action === "mark_received" &&
          !isOwner &&
          !["approved", "ordered"].includes(ord?.status ?? "")
        ) {
