...
export const generateInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    await supabase.from("audit_log").insert({
      ...
      entity: "invite_code",
      entity_id: row.id,
      payload: { code: row.code, role: data.role, expires_hours: data.expiresHours },
    });
    return row;
  });
...
export const listInvitesV2 = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("invite_codes")
      .select(
        "id, code, role, note, trailer_id, expires_at, expires_hours, used_by, used_at, disabled_at, created_at",
      )
...
export const setUserActive = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    await supabaseAdmin.from("access_log").insert({
      user_id: data.userId,
      event: data.active ? "access_restored" : "access_revoked",
      payload: { by: userId },
    });
    return { ok: true };
  });
