  ...
  .handler(async ({ context }) => {
    ...
    if (myShiftIds.length === 0) {
      ...
      const { data } = await supabase
        .from("tasks")
        .select(
          "id, title, description, phase, status, assignee_role, assignee_user_id, requires_signoff, completed_at, signed_off_at, trailer_id, shift_id, created_at",
        )
...
export const completeTask = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: task0, error: te } = await supabase
      .from("tasks")
      .select("id, assignee_user_id, assignee_role, trailer_id")
      .eq("id", data.taskId)
      .maybeSingle();
    if (te) throw te;
...
export const signOffTask = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.approve) {
      ...
      await supabase
        .from("audit_log")
        .insert({ actor_id: userId, action: "signoff_task", entity: "task", entity_id: task.id });
      return task;
    } else {
      const { data: task, error } = await supabase
        .from("tasks")
      ...
      await supabase
        .from("audit_log")
        .insert({ actor_id: userId, action: "reject_task", entity: "task", entity_id: task.id });
      return task;
    }
  });
