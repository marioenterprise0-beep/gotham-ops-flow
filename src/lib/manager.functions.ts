...
export const getManagerOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const profilesQ = trailerId
      ? supabase
          .from("profiles")
          .select("id, display_name, trailer_id")
    ...
    const itemsQ = trailerId
      ? supabase
          ...
          .select("current_qty, low_threshold, par_level")
          .eq("trailer_id", trailerId)
          .is("archived_at", null)
      : supabase
          .from("inventory_items")
    ...
    if (shift) {
      ...
      teamScore = (profiles ?? []).length
        ? Math.round((active / (profiles ?? []).length) * 100)
        : 0;
    ...
    const penalty = (incidents ?? []).reduce(
      (acc, i) => acc + (i.severity === "high" ? 15 : i.severity === "medium" ? 7 : 3),
      0,
    );
    ...
    let recapsQ = supabase
      .from("daily_recaps")
      .select("id", { count: "exact", head: true })
      .is("archived_at", null)
      .eq("status", "submitted");
    if (trailerId) recapsQ = recapsQ.eq("trailer_id", trailerId);
    const [{ count: pendingAlerts }, { count: pendingRecaps }] = await Promise.all([
      alertsQ,
      recapsQ,
    ]);
...
export const createActionTask = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    try {
      ...
      const desc =
        data.description ??
        `Phase${phaseLabel}${data.requiresSignoff ? " · requires sign-off" : ""}`;
      if (data.assigneeUserId) {
...
export const reorderItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: item } = await supabase
      .from("inventory_items")
      .select("name, par_level, current_qty, unit, store_id, trailer_id")
      .eq("id", data.itemId)
      .single();
...
export const listCrewRoster = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const profQ = trailerId
      ? supabase
          .from("profiles")
          .select("id, display_name, created_at, trailer_id")
          .eq("trailer_id", trailerId)
          .is("archived_at", null)
          .order("display_name")
      : supabase
...
export const listAuditLog = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const ids = Array.from(
      new Set((data ?? []).map((r) => r.actor_id).filter((x): x is string => !!x)),
    );
    ...
    return (data ?? []).map((r) => ({
      ...r,
      actor_name: r.actor_id ? (nameById.get(r.actor_id) ?? "Unknown") : "System",
    }));
  });
