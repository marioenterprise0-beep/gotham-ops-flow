...
export const getMyProfile = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: profile }, { data: roles }, { data: store }] = await Promise.all([
      supabase
        .from("profiles")
        .select("id, display_name, store_id, created_at")
        .eq("id", userId)
        .maybeSingle(),
      ...
      supabase
        .from("stores")
        .select(
          "id, name, short_name, tagline, support_email, location, bg_color, fg_color, accent_color",
        )
        .order("created_at")
        .limit(1)
        .maybeSingle(),
...
export const sendBrandingTestEmail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [{ TEMPLATES }, { applyBrandOverrides }, { render }, Rreact] = await Promise.all([
    ...
    const templateName =
      data.templateName && TEMPLATES[data.templateName]
        ? data.templateName
