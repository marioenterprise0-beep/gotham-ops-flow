...
export const createInventoryCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: row, error } = await supabase
      ...
      .single();
    if (error)
      throw new Error(error.code === "23505" ? `Category "${key}" already exists` : error.message);
...
export const archiveInventoryCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { count } = await (supabase as any)
      ...
      .is("archived_at", null);
    if ((count ?? 0) > 0)
      throw new Error(`Move or archive the ${count} item(s) in this category first.`);
    ...
    await supabase.from("audit_log").insert({
      actor_id: userId,
      action: "inventory_category_archived",
      entity: "inventory_category",
      entity_id: data.id,
...
export const upsertInventoryItem = createServerFn({ method: "POST" })
    ...
    z
      .object({
        ...
        imageUrl: z
          ...
          .max(1000)
          .refine((v) => !v.includes("://"), {
            message: "imageUrl must be a storage path, not a URL",
          })
          .nullable()
          .optional(),
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.preferredOrderQty !== undefined)
      structural.preferred_order_qty = data.preferredOrderQty;
    ...
    if (data.countInstructions !== undefined)
      structural.count_instructions = data.countInstructions;
    if (data.storageLocation !== undefined) structural.storage_location = data.storageLocation;
    if (data.archived !== undefined)
      structural.archived_at = data.archived ? new Date().toISOString() : null;
    ...
    await supabase.from("audit_log").insert({
      actor_id: userId,
      action: data.id ? "update_item" : "create_item",
      entity: "inventory_item",
      entity_id: data.id ?? null,
      payload: { name: data.name, category, trailer_id: trailerId, propagated: true },
    });
    return { ok: true };
  });
...
export const updateOrderGuide = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    await supabase.from("audit_log").insert({
      ...
      entity: "inventory_item",
      entity_id: data.id,
      payload: { ...data.patch, propagated: true },
    });
    return { ok: true };
  });
  ...
  .handler(async ({ context, data }) => {
    ...
    if (item) {
      ...
      const { error: upErr } = await supabaseAdmin
        .from("inventory_items")
        .update({ current_qty: Number(item.current_qty) + data.qty })
        .eq("id", data.itemId);
