...
async function getTrailerId(supabase: any, userId: string, trailerId?: string | null): Promise<string | null> {
...
export const logPrepEntry = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: activeShift } = trailerId
      ? await supabase.from("shifts").select("id").eq("trailer_id", trailerId).eq("status", "active")
    ...
    await supabase.from("audit_log").insert({
      actor_id: userId, action: "prep_logged", entity: "prep_log",
      entity_id: row.id, payload: { item: data.itemName, qty: data.quantity, unit: data.unit },
    });
    return row;
  });
...
export const deletePrepEntry = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const { data: entry } = await supabase.from("prep_log").select("logged_by").eq("id", data.id).maybeSingle();
    if (!entry) throw new Error("Entry not found");
    const { data: roles } = await supabase.from("user_roles").select("role").eq("user_id", userId);
    const isManager = (roles ?? []).some((r: any) => r.role === "owner" || r.role === "manager");
    if (!isManager && (entry as any).logged_by !== userId) throw new Error("You can only delete your own entries");
    const { error } = await supabase.from("prep_log")
