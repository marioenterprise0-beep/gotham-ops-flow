...
import { z } from "zod";
import { ARCHIVE_DOMAINS, DOMAIN_BY_TABLE, DEFAULT_RETENTION_DAYS, type ArchiveDomain } from "./archive-registry";
...
export const runAllDependencyScans = createServerFn({ method: "POST" })
  ...
  .inputValidator((d) => z.object({ retentionDays: z.number().int().min(1).max(3650).default(DEFAULT_RETENTION_DAYS) }).parse(d))
  .handler(async ({ context, data }) => {
    ...
    const results = await Promise.all(ARCHIVE_DOMAINS.map(async (domain) => {
      const { count: totalArchived } = await sb.from(domain.table)
        .select("id", { count: "exact", head: true }).not("archived_at", "is", null);
      const { data: oldestRow } = await sb.from(domain.table)
        .select("archived_at").not("archived_at", "is", null).order("archived_at", { ascending: true }).limit(1);
      ...
      const { data: candidates } = await sb.from(domain.table)
        .select(cols.join(", ")).not("archived_at", "is", null).lt("archived_at", cutoff).limit(200);

      let blocked = 0;
      let purgeable = 0;
      const blockedSamples: Array<{ id: string; name: string }> = [];
      for (const r of candidates ?? []) {
        if (domain.deps.length === 0) { purgeable++; continue; }
        const live = await rowHasLiveDeps(sb, domain, r.id);
        if (live) {
          blocked++;
          if (blockedSamples.length < 5) {
            blockedSamples.push({ id: r.id, name: domain.nameColumn ? r[domain.nameColumn] ?? r.id.slice(0, 8) : r.id.slice(0, 8) });
          }
        } else {
          purgeable++;
        }
