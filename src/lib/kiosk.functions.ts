...
export const listKioskDevices = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("trusted_clock_devices")
      .select("id, trailer_id, label, approved_at, approved_by, revoked_at, revoked_by, last_used_at, created_at")
...
export const setKioskDeviceRequired = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: existing } = await supabaseAdmin
      .from("automation_settings").select("id").eq("scope", "global").maybeSingle();
    if (existing?.id) {
      const { error } = await supabaseAdmin.from("automation_settings")
        .update({ kiosk_device_required: data.enabled }).eq("id", existing.id);
      if (error) throw new Error(error.message);
    } else {
      const { error } = await supabaseAdmin.from("automation_settings")
...
export const kioskWhoAmI = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: trailer } = await supabaseAdmin
      .from("trailers").select("id, name").eq("id", device.trailerId).maybeSingle();
    return {
      ok: true as const,
...
export const kioskListEmployees = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [openPunches, pins] = await Promise.all([
      supabaseAdmin.from("time_punches")
      ...
      supabaseAdmin.from("employee_pins")
        .select("user_id")
...
export const kioskClockIn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: todaysShift } = await supabaseAdmin.from("schedule_shifts")
      .select("id").is("archived_at", null)
      .eq("employee_id", data.employeeId).eq("shift_date", today)
      .order("start_time").limit(1).maybeSingle();

    const deviceInfo = { ...deviceInfoGeo, source: "kiosk", device_id: device.id, device_label: device.label };
    const { data: row, error } = await supabaseAdmin.from("time_punches").insert({
    ...
    }).select("id, clock_in_at").single();
    if (error) return { ok: false as const, message: error.message };

    await supabaseAdmin.from("trusted_clock_devices")
...
export const kioskClockOut = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: open } = await supabaseAdmin.from("time_punches")
      .select("id").eq("employee_id", data.employeeId).eq("status", "open").is("archived_at", null)
    ...
    const { error } = await supabaseAdmin.from("time_punches")
      .update(patch).eq("id", open.id);
    if (error) return { ok: false as const, message: error.message };
