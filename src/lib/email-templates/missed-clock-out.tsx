...
const Email = ({ recipient_name, employee_name, shift_date, scheduled_end, auto_closed, punch_id }: Props) => (
  <BrandLayout preview={`Missed clock-out — ${employee_name ?? ''}`}>
    ...
    <Text style={styles.text}>
      {recipient_name ? `${recipient_name}, ` : ''}{employee_name ?? 'an employee'} did not clock out at the end of their shift on {shift_date ?? '—'}.
    </Text>
    <DataTable>
    ...
    <OpenGothamButton path={punch_id ? `/time/punches/${punch_id}` : '/time'} label="Open Time Record" />
  </BrandLayout>
)
...
export const template = {
  ...
  previewData: { employee_name: 'Sara Ahmed', shift_date: 'Mon Dec 9', scheduled_end: '6:00 PM', auto_closed: true },
