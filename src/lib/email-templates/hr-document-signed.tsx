...
interface Props { recipient_name?: string; title?: string; employee_name?: string; completed_at?: string }
const Email = ({ recipient_name, title, employee_name, completed_at }: Props) => (
  <BrandLayout preview={`Document fully signed — ${title ?? ''}`}>
    ...
    <Text style={styles.text}>{recipient_name ? `${recipient_name}, ` : ''}<strong>{title ?? 'A document'}</strong> for {employee_name ?? 'an employee'} now has every required signature.</Text>
    <DataTable>
...
export const template = { component: Email, subject: (d: any) => `Document fully signed — ${d?.title ?? ''}`, displayName: 'HR Document Signed', previewData: { title: 'Written Warning Form', employee_name: 'Sara Ahmed', completed_at: 'Thu Dec 12 4:20 PM' } } satisfies TemplateEntry
