...
const Email = ({
  ...
  <BrandLayout preview={`Cash variance — ${drawer_name ?? ""}`}>
    ...
    <Text style={styles.text}>
      {recipient_name ? `${recipient_name}, ` : ""}a cash variance was reported on{" "}
      {drawer_name ?? "a drawer"} by {submitted_by ?? "the closer"}. Please review immediately.
    </Text>
    <DataTable>
    ...
    <OpenGothamButton
      path={session_id ? `/cash/sessions/${session_id}` : "/cash"}
...
export const template = {
  component: Email,
  subject: (d: any) => `URGENT cash variance — ${d?.drawer_name ?? ""}`,
  displayName: "Cash Variance Alert",
  previewData: {
    drawer_name: "Drawer A",
    expected: "420.00",
    counted: "375.00",
    variance: "-45.00",
    reason: "Refund miscounted",
    submitted_by: "Aisha",
  },
