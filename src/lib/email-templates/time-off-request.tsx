...
const Email = ({ recipient_name, employee_name, start_date, end_date, reason, request_id }: Props) => (
  <BrandLayout preview={`Time off request — ${employee_name ?? ''}`}>
    ...
    <Text style={styles.text}>
      {recipient_name ? `${recipient_name}, ` : ''}{employee_name ?? 'an employee'} requested time off
      {start_date ? ` for ${start_date}${end_date && end_date !== start_date ? ` – ${end_date}` : ''}` : ''}.
    </Text>
    <DataTable>
    ...
    <OpenGothamButton path={request_id ? `/time-clock?request=${request_id}` : '/time-clock'} label="Review Request" />
  </BrandLayout>
)
...
export const template = {
  ...
  previewData: { employee_name: 'Sara Ahmed', start_date: 'Mon Dec 9', end_date: 'Wed Dec 11', reason: 'Family event' },
