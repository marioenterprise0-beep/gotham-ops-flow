...
const Email = ({ recipient_name, shift_date, approved_value, approver_name, punch_id }: Props) => (
  <BrandLayout preview="Time adjustment approved">
    ...
    <Text style={styles.text}>
      {recipient_name ? `Hi ${recipient_name}, your` : 'Your'} correction for {shift_date ?? 'the requested shift'} has been approved{approver_name ? ` by ${approver_name}` : ''}. Your weekly hours have been updated.
    </Text>
    <DataTable>
    ...
    <OpenGothamButton path={punch_id ? `/time/punches/${punch_id}` : '/time'} label="View Time Record" />
  </BrandLayout>
)
...
export const template = {
  ...
  previewData: { recipient_name: 'Sara', shift_date: 'Mon Dec 9', approved_value: '10:00 AM – 6:15 PM', approver_name: 'Omar' },
