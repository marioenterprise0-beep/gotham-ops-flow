...
interface Props { recipient_name?: string; sop_title?: string; due_date?: string; assigned_by?: string; sop_id?: string }
const Email = ({ recipient_name, sop_title, due_date, assigned_by, sop_id }: Props) => (
  <BrandLayout preview={`Training assigned — ${sop_title ?? ''}`}>
    ...
    <Text style={styles.text}>{recipient_name ? `Hi ${recipient_name}, ` : ''}you've been assigned training for <strong>{sop_title ?? 'an SOP'}</strong>{assigned_by ? ` by ${assigned_by}` : ''}.</Text>
    <DataTable>
...
export const template = { component: Email, subject: (d: any) => `Training assigned — ${d?.sop_title ?? ''}`, displayName: 'Training Assigned', previewData: { recipient_name: 'Sara', sop_title: 'Drawer Closing SOP', due_date: 'Fri Dec 13', assigned_by: 'Aisha' } } satisfies TemplateEntry
