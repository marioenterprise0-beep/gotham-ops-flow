...
interface Shift { date?: string; start?: string; end?: string; role?: string }
interface Props {
...
const Email = ({ recipient_name, week_range, location, shifts = [], total_hours, schedule_id }: Props) => (
  <BrandLayout preview={`Your schedule — ${week_range ?? ''}`}>
    ...
    <Text style={styles.text}>
      Hi {recipient_name ?? 'team'}, your shifts for {week_range ?? 'the upcoming week'} at {location ?? 'your location'} are now confirmed.
    </Text>
    <DataTable>
