...
const Email = ({
  ...
  <BrandLayout preview={`Unavailability request — ${employee_name ?? ''}`}>
    ...
    <Text style={styles.text}>
      ...
      {schedule_status === 'locked' ? ' locked' : ' published'}, so their availability
    ...
    <OpenGothamButton path={request_id ? `/labor?availability=${request_id}` : '/labor'} label="Review Request" />
  </BrandLayout>
)
