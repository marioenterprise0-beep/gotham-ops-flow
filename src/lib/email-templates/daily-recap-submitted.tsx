...
interface Props { recipient_name?: string; manager_name?: string; recap_date?: string; location?: string; shift_score?: number | string; highlights?: string; recap_id?: string }
const Email = ({ recipient_name, manager_name, recap_date, location, shift_score, highlights, recap_id }: Props) => (
  <BrandLayout preview={`Daily recap — ${recap_date ?? ''}`}>
    ...
    <Text style={styles.text}>{recipient_name ? `${recipient_name}, ` : ''}{manager_name ?? 'A manager'} submitted the daily recap for {recap_date ?? 'today'}.</Text>
    <DataTable>
      ...
      <DataRow label="Shift score" value={shift_score != null ? `${shift_score} / 10` : '—'} emphasis />
    </DataTable>
