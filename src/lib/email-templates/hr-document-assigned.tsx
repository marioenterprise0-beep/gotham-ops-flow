...
interface Props { recipient_name?: string; title?: string; due_date?: string; assigned_by?: string }
const Email = ({ recipient_name, title, due_date, assigned_by }: Props) => (
  <BrandLayout preview={`New document to review — ${title ?? ''}`}>
    ...
    <Text style={styles.text}>{recipient_name ? `Hi ${recipient_name}, ` : ''}you've been sent <strong>{title ?? 'a document'}</strong>{assigned_by ? ` by ${assigned_by}` : ''} to view and sign.</Text>
    <DataTable>
...
export const template = { component: Email, subject: (d: any) => `New document to review — ${d?.title ?? ''}`, displayName: 'HR Document Assigned', previewData: { recipient_name: 'Sara', title: 'Tip Pool Acknowledgement', due_date: 'Fri Dec 13', assigned_by: 'Aisha' } } satisfies TemplateEntry
