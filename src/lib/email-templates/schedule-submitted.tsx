...
const Email = ({ recipient_name, manager_name, week_range, location, total_hours, schedule_id }: Props) => (
  <BrandLayout preview={`Schedule submitted for approval — ${week_range ?? ''}`}>
    ...
    <Text style={styles.text}>
      {recipient_name ? `${recipient_name}, ` : ''}{manager_name ?? 'A manager'} submitted a new schedule that needs your review before it can be published.
    </Text>
    <DataTable>
      ...
      <DataRow label="Total scheduled hours" value={total_hours != null ? `${total_hours} hrs` : '—'} />
    </DataTable>
    <OpenGothamButton path={schedule_id ? `/schedule/${schedule_id}` : '/schedule'} label="Review Schedule" />
  </BrandLayout>
)
