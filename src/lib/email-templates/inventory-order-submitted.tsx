...
const Email = ({
  ...
  return (
    <BrandLayout preview={`Inventory order submitted — ${loc ?? ""}`}>
      <StatusBadge
        variant={critical_count && critical_count > 0 ? "critical" : "warning"}
        label={
          critical_count && critical_count > 0 ? "Critical · Owner Review" : "Pending Owner Review"
        }
      />
      <Heading style={styles.h1}>Inventory order submitted</Heading>
      <Text style={styles.text}>
        {recipient_name ? `${recipient_name}, ` : ""}
        {submitted_by ?? "A manager"} submitted an inventory order for your review.
      </Text>
      <DataTable>
      ...
      <OpenGothamButton
        path={order_id ? `/inventory/orders/${order_id}` : "/inventory/orders"}
        label="Open Inventory Order"
      />
    </BrandLayout>
  );
};
export const template = {
  component: Email,
  subject: (d: any) => `Inventory order submitted — ${d?.location ?? d?.trailer_name ?? ""}`,
  displayName: "Inventory Order Submitted",
  previewData: {
    submitted_by: "Aisha",
    location: "Location 1",
    item_count: 3,
    ...
    items: [
      ...
      { item_name: "Pita bread", requested_qty: 80, unit: "ea", current_qty: 12, par_qty: 80 },
      { item_name: "Tahini", requested_qty: 6, unit: "jar" },
    ],
  },
