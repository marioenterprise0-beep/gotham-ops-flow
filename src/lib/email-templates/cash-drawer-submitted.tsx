...
const Email = ({
  ...
  return (
    <BrandLayout preview={`Cash drawer submitted — ${drawer_name ?? ""}`}>
      <StatusBadge
        variant={isVar ? "warning" : "success"}
        label={isVar ? "Variance Detected" : "Balanced"}
      />
      <Heading style={styles.h1}>Cash drawer submitted</Heading>
      <Text style={styles.text}>
        {recipient_name ? `${recipient_name}, ` : ""}
        {submitted_by ?? "A manager"} closed the cash drawer and submitted the count.
      </Text>
      <DataTable>
...
export const template = {
  component: Email,
  subject: (d: any) => `Cash drawer submitted — ${d?.drawer_name ?? ""}`,
  displayName: "Cash Drawer Submitted",
  previewData: {
