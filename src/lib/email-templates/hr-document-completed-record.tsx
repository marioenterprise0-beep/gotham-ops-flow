...
const Email = ({ employee_name, title, completed_at, pdf_url }: Props) => (
  <BrandLayout preview={`Completed record — ${title ?? ""} — ${employee_name ?? ""}`}>
    ...
    <Text style={styles.text}>
      <strong>{title ?? "A document"}</strong> for {employee_name ?? "an employee"} has every
      required signature. A copy of the completed, filled-in document is attached below for your
      records.
    </Text>
    <DataTable>
...
export const template = {
  component: Email,
  subject: (d: any) =>
    `[Dip N Shake] Completed: ${d?.title ?? "HR Document"} — ${d?.employee_name ?? ""}`,
