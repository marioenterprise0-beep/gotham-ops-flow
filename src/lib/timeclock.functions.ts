  ...
  .handler(async ({ context, data }) => {
    ...
    // would otherwise see null and bypass the guard.
    {
      ...
      if (settings?.kiosk_device_required) {
        return {
          ok: false as const,
          message:
            "Clock-in is kiosk-only. Please use the location iPad. If it's unavailable, ask a manager to clock you in.",
        };
      }
    }
    ...
    if (!scheduleShiftId) {
      ...
      const { data: todaysShift } = await supabase
        ...
        .select("id, schedules!inner(archived_at)")
    ...
    if (trailerId) {
      try {
        ...
        if (ensured) {
          ...
          assignedTaskCount = await instantiatePersonalTasks(
            supabase,
            ensured.shift.id,
            trailerId,
            userId,
...
export const setTrailerGeofence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: rows, error: readErr } = await supabaseAdmin.rpc("get_trailer_geofence", {
      _trailer_id: data.trailerId,
    });
...
export const geocodeAddress = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const res = await fetch(url, {
      headers: { "User-Agent": "gotham-os/1.0 (timeclock-geofence)" },
    });
...
export const clockOut = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    // automation_settings SELECT is manager-only (crew would see null and bypass).
    {
      ...
      if (settings?.kiosk_device_required) {
        throw new Error(
          "Clock-out is kiosk-only. Please use the on-site iPad, or ask a manager to close your punch.",
        );
      }
    }
...
export const listPunchesForAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = supabase
      .from("time_punches")
      .select(
        "id, employee_id, trailer_id, clock_in_at, clock_out_at, break_minutes, status, notes, schedule_shift_id",
      )
      .is("archived_at", null)
...
export const managerClockInEmployee = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!data.clockOutAt) {
      ...
      if (existing)
        throw new Error(
          "Employee already has an open punch. Close it first or set a clock-out time.",
        );
    }
    ...
    const scheduleShiftId = await findMatchingScheduleShift(
      supabase,
      data.employeeId,
      trailerId,
      data.clockInAt,
...
export const managerClockOutEmployee = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: existingPunch } = await supabase
      ...
      .maybeSingle();
    const patch: any = {
      clock_out_at: addDayIfBefore(existingPunch?.clock_in_at ?? data.clockOutAt, data.clockOutAt),
      status: "closed",
    };
    if (data.breakMinutes !== undefined) patch.break_minutes = data.breakMinutes;
...
export const managerEditPunch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (
      !patch.schedule_shift_id &&
      existingPunch?.employee_id &&
      finalClockIn &&
      !existingPunch.schedule_shift_id
    ) {
      const matchedShift = await findMatchingScheduleShift(
        supabase,
        existingPunch.employee_id,
        existingPunch.trailer_id ?? null,
        finalClockIn,
      );
